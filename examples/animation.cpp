...
struct animation_logger {
    ...
    // }
    template<typename Guard> void log_guard(Animation const&, Guard const&, bool result) const noexcept
    {
        std::cerr << "[" << ufsm::logging::get_type_name<Animation>() << "]: guard ["
                  << ufsm::logging::get_type_name<Guard>() << "]" << (result ? " [OK]" : " [Reject]") << "\n";
    }
    template<typename Action, typename Event>
    void log_action(Animation const&, Action const&, Event const&) const noexcept
    {
        std::cerr << "[" << ufsm::logging::get_type_name<Animation>() << "]: action ["
                  << ufsm::logging::get_type_name<Action>() << "]\n";
    }
    template<typename State> void log_exit(Animation const&, State const&) const noexcept
    {
        std::cerr << "[" << ufsm::logging::get_type_name<Animation>() << "]: state exit ["
                  << ufsm::logging::get_type_name<State>() << "]\n";
    }
    template<typename State> void log_entry(Animation const&, State const&) const noexcept
    {
        std::cerr << "[" << ufsm::logging::get_type_name<Animation>() << "]: state entry ["
                  << ufsm::logging::get_type_name<State>() << "]\n";
    }
    ...
    {
        std::cerr << "[" << ufsm::logging::get_type_name<Animation>() << "]: state change ["
                  << ufsm::logging::get_type_name<SrcState>() << "] -> [" << ufsm::logging::get_type_name<DstState>()
                  << "]\n";
    }
};
...
template<template<class...> class C, typename... Ts> struct count<C<Ts...>> {
    static constexpr inline auto value = sizeof...(Ts);
};

template<typename List> static constexpr inline auto count_v = count<List>::value;
...
class Animation  //: public ufsm::Fsm<Animation, sAnimating, sPaused, sIdle>
{
  public:
    ...
    struct guard1 {
        template<typename Event> constexpr bool operator()(Event const&) const noexcept { return true; }
    };
    struct action1 {
        template<typename Event> void operator()(Event&&) const
        {
    ...
    struct guard2 {
        ...
        template<typename Event> constexpr bool operator()(Event const&) const noexcept
        {
...
{
    std::cerr << '[' << ufsm::logging::get_type_name<Animation>() << "]: count = " << fsm.counter << " in state ["
              << ufsm::logging::get_type_name<State>() << "], got event [" << ufsm::logging::get_type_name<Event>()
              << "]\n";
}
...
struct testaction {
    template<typename Event> inline void operator()(Animation const&, Event&& event) const
    {
        std::cerr << "testaction\n";
    }
};
...
template<typename T> inline void run_animation(T&& fsm) noexcept
{
