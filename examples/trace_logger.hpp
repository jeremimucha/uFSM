...
template<typename FsmType> struct trace_logger {
    template<typename State, typename Event> void log_event(FsmType const&, State const&, Event const&) const noexcept
    {
    ...
    template<typename State> void log_exit(FsmType const&, State const&) const noexcept
    {
        std::cerr << "[" << ufsm::logging::get_type_name<FsmType>() << "]: state exit ["
                  << ufsm::logging::get_type_name<State>() << "]\n";
    }
    template<typename State> void log_entry(FsmType const&, State const&) const noexcept
    {
