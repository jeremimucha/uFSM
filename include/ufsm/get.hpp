...
template<typename State, SizeT Idx>
inline constexpr FsmStateT<Idx, State>& get_impl(FsmState<Idx, State>& state) noexcept
{
    return state.get();
}
template<typename State, SizeT Idx>
inline constexpr FsmStateT<Idx, State> const& get_impl(FsmState<Idx, State> const& state) noexcept
{
    return state.get();
}
template<typename State, SizeT Idx>
