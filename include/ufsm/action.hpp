...
template<typename T, typename... Args>
struct HasActionT<T, void_t<transition_action_call<T, Args...>>, Args...> : std::true_type { };
...
struct SelectTransitionActionT {
    using type = NoTransitionAction;
};
...
struct SelectTransitionActionEventT<TTraits, Event, void_t<transition_action_call<TTraits, Event>>> {
...
struct SelectTransitionActionFsmEventT<TTraits, FsmT, Event, void_t<transition_action_call<TTraits, FsmT, Event>>> {
    using type = TransitionActionFsmEvent;
};


template<typename TTraits, typename FsmT, typename Event>
using SelectTransitionActionSignature = typename SelectTransitionActionFsmEventT<TTraits, FsmT, Event>::type;

template<typename Indices>
struct executeAction;

template<typename Indices>
struct propagateActionImpl;
template<>
struct propagateActionImpl<IndexSequence<>> {
...
struct propagateAction<State, true> {
    ...
    {
        propagateActionImpl<GetIndices<FsmT>>{}(std::forward<FsmT>(fsm), std::forward<Event>(event));
    }
};
...
struct fsmAction {
    ...
    {
        /* nop */
        detail::propagateAction<std::decay_t<State>>{}(std::forward<State>(state), std::forward<Event>(event));
    }
};
...
constexpr inline void fsm_action(TTraits&& ttraits, FsmT&& fsm, State&& state, Event&& event) noexcept
{
    ...
    fsmAction<ttraits_t, fsm_t, event_t>{}(
      std::forward<TTraits>(ttraits), std::forward<FsmT>(fsm), std::forward<State>(state), std::forward<Event>(event));
}

namespace detail {
template<>
struct executeAction<IndexSequence<>> {
...
struct executeAction<IndexSequence<I, Is...>> {
    template<typename TraitsTuple, typename FsmT, typename State, typename Event>
      ...
      operator()(TraitsTuple&& traits_tuple, FsmT&& fsm, State&& state, Event&& event) const noexcept
    {
        ...
        {
            fsmAction<traits_t, FsmT, Event>(std::forward<decltype(traits)>(traits),
        ...
        {
            executeAction<IndexSequence<Is...>>{}(std::forward<TraitsTuple>(traits_tuple),
                                                  std::forward<FsmT>(fsm),
                                                  std::forward<State>(state),
                                                  std::forward<Event>(event));
        }
...
template<SizeT I, SizeT... Is>
struct propagateActionImpl<IndexSequence<I, Is...>> {
    template<typename FsmT, typename Event>
    constexpr inline void operator()(FsmT&& fsm, Event&& event) const noexcept
    {
        if (I == fsm.state())
        {
            ...
            auto&& traits_tuple{getTransitionTraits<state_t, event_t>(fsm.transition_table())};
            // auto&& or decltype(auto) ?
            auto&& state{get<I>(fsm)};
            using Indices = MakeIndexSequence<std::tuple_size_v<std::decay_t<decltype(traits_tuple)>>>;
            executeAction<Indices>{}(std::forward<decltype(traits_tuple)>(traits_tuple),
                                     std::forward<FsmT>(fsm),
        ...
        {
            propagateActionImpl<IndexSequence<Is...>>{}(std::forward<FsmT>(fsm), std::forward<Event>(event));
        }
    }
};
}  // namespace detail
